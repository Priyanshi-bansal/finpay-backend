...
const payIn = async(req, res)=>{
    const {amount, reference, name, mobile, email} = req.body;

    if(!amount || !reference || !name || !mobile || !email){
        return res.send("All fields are required");
    }
    const payInData = await axios.post('https://api.worldpayme.com/api/v1.1/createUpiIntent',
        {
            amount, reference, name, mobile, email
        },
        {
            headers:{
    ...
    if(payInData.data){
        return res.status(200).send({data:payInData.data, status:payInData.data.status});
    }else{
...
const payOut = async(req, res)=>{
    const {amount, reference, trans_mode, account, ifsc, name, mobile, email, address} = req.body;

    if(!amount || !reference || !name || !mobile || !email||!trans_mode){
        return res.send("All fields are required");
    }
    const payOutData = await axios.post('https://api.worldpayme.com/api/v1.1/payoutTransaction',
        {
    ...
    if(payOutData.data){
        return res.status(200).send({data:payOutData.data, status:payOutData.data.status});
    }else{
        return res.status(400).send("bad request");
    }
}
