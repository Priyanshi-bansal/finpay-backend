...
try {
  ...
  const results = await Promise.all([
    ...
    PayOut.aggregate([
      ...
      { $group: { _id: null, totalApproved: { $sum: { $toDouble: "$amount" } } } }
    ]),

    // Pending
    PayOut.aggregate([
      { $match: { userId: new mongoose.Types.ObjectId(userId), status: "Pending" } },
      { $group: { _id: null, totalPending: { $sum: { $toDouble: "$amount" } } } }
    ]),

    // Failed
    PayOut.aggregate([
      { $match: { userId: new mongoose.Types.ObjectId(userId), status: "Failed" } },
      { $group: { _id: null, totalFailed: { $sum: { $toDouble: "$amount" } } } }
    ])
  ]);
...
module.exports = { payOut, adminAction, callbackPayout, payOutReportAllUsers, payOutReportForUser, payOutUserCount };
